...
| 任务 | 按班级编辑任务;每条=内容+备注(如请假方式说明,显示在内容下方)+时间点(如 `12:30`)或时间段(`12:30-13:30`);日期模式:**仅一次 / 连续日期范围 / 每周重复(选星期几)/ 每天**;可单独勾选“结束时全屏提醒”并填写可选提示文本;某天任务可一键复制到其他天;可停用/删除 |
...
- 半透明深色圆角面板,贴在桌面右侧(可改左侧/顶部)。**除任务列表外的区域鼠标完全穿透**——桌面图标、右键菜单一切如常;任务较多时列表区域可按住拖动或滚动滚轮查看,停止操作 10 秒后自动回到当前/即将开始的条目(上下居中,首尾自动贴边)。
- **永远位于壁纸之上、所有应用窗口之下**:老师打开 PPT、白板软件等任何程序都会自然盖住看板,不影响上课;应用关闭后看板自然露出来。Win+D 显示桌面后自动恢复。
- 显示内容:大时钟(时:分:秒)、日期+星期+班级、今日任务时间轴(当前进行中的条目高亮"进行中"、已过的变暗)、下一条倒计时("接下来 12:30 去食堂吃饭 · 还有 8 分钟";当天只剩进行中的任务时显示"当前进行中 · HH:MM 结束")、顶部通知栏。
- 断网/服务器宕机时看板照常显示(本地缓存),并在面板上提示"离线";网络恢复自动重连(3s→10s→30s 退避)。
- 教室电脑时钟不准也没关系:客户端以服务器时间为准。

### 任务结束全屏提醒

- 每个任务可独立启用。**时间点任务在开始时间提醒，时间段任务在结束时间提醒**。
- 提醒窗口覆盖主屏幕并始终置顶，使用适合希沃大屏远距离阅读的大字号，显示任务名、时间、日期、班级、任务备注和可选提示文本。
- 提醒持续 5 分钟后自动退出；点击“立刻退出”需要输入客户端管理密码（与托盘退出使用同一密码）。
- 同一任务同一天的同一提醒时刻只提醒一次；当天调整任务时间后，会按新时刻重新提醒。同一时刻有多项任务时依次展示。客户端在触发后 5 分钟内启动或恢复运行时仍会补发，超过 5 分钟不再打扰。
...
- **向后兼容**:旧版客户端连到新服务器会忽略新增字段,仍正常显示任务但不会执行全屏提醒;新版客户端读取旧缓存或旧任务时默认关闭提醒。数据库升级(任务备注、提醒开关与提示文本列)在服务器启动时自动完成。
...
- [ ] 任务多时:列表可拖动/滚轮滚动;停止操作 10 秒后自动回到当前或即将开始的任务;最后任务进行中时倒计时显示"当前进行中"而非"已全部完成"
- [ ] 网站叫号:≤3 秒弹窗、响铃、10 秒自动关;教师端状态变"已显示→已关闭"
- [ ] 系统静音状态下叫号:自动取消静音并响铃
- [ ] 断网 1 分钟:看板照常显示并提示离线;恢复网络后自动重连
- [ ] 网站远程改弹窗时长/音量:客户端 3 秒内生效
- [ ] 分别创建时间点、时间段任务并启用全屏提醒:在开始/结束时刻全屏置顶显示
- [ ] 全屏提醒“立刻退出”密码错误时保持显示，密码正确时关闭；不操作时 5 分钟自动关闭
- [ ] 托盘"退出"需要密码;密码错误退不出
- [ ] 重启电脑:客户端自启且无需重新配对
