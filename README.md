...
> **Win7 客户端 + HTTPS 的注意**:Win7 需已安装 .NET 4.8 并启用系统 TLS1.2(保持 Windows Update 的最新 Win7 SP1 默认可用;若叫号/配对提示 SSL/TLS 通道创建失败,在注册表 `HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Internet Settings\WinHttp` 加 DWORD `DefaultSecureProtocols = 0x00000A00` 后重启)。
...
| 任务 | 按班级编辑任务;每条=内容+备注(如请假方式说明,显示在内容下方)+时间点(如 `12:30`)或时间段(`12:30-13:30`);日期模式:**仅一次 / 连续日期范围 / 每周重复(选星期几)/ 每天**;某天任务可一键复制到其他天;可停用/删除 |
| 叫号 | 号数(可一次多个,如 `12, 15, 27`)+ 地点(默认"办公室",有快捷按钮可自定义)+ 原因(可空);历史记录实时显示状态:待接收 → 已显示 → 已关闭,可撤销未接收的叫号 |
| 客户端 | 建班级、生成配对码(6 位,30 分钟内有效)、给客户端命名/换绑班级/解绑;**远程下发设置**:弹窗时长(3-120 秒)、铃声音量、看板位置(右/左/顶)、字号缩放,客户端 3 秒内生效;**发布客户端新版本**(见下文"自动更新") |
...
1. 在自己电脑上用 `client/build.bat` 编译出新的 `MultimediaClient.exe`(版本号在 `client/src/AppHost.cs` 的 `Version` 常量里,注意改成更大的 `x.y.z`)。
2. 教师网站 →「客户端」页 → 最下方「客户端版本发布」:填版本号、选择 exe → 上传发布。
3. 所有在线客户端约 3 秒内检测到新版本,**自动下载 → 校验 → 重启完成升级**,全程无需人到教室。

可靠性与兼容性设计:

- 下载完成后先做 **sha256 + 大小 + PE 头三重校验**,校验不过绝不替换,30 分钟后重试。
- 替换前自动备份旧程序为 `MultimediaClient.exe.bak`,替换由独立批处理在程序退出后完成,不会出现"文件被占用"。
- **向后兼容**:旧版客户端连到新服务器会忽略版本字段,不受影响;新版客户端连到未发布过版本的服务器则不会尝试更新。数据库升级(如任务备注列)在服务器启动时自动完成。
- 防循环保护:已应用过的版本号会记录在客户端配置里,即使误传了错误的安装包也不会反复重启。
